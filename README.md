# Exploit Validation & Remediation MCP Server

An industry-standard **Model Context Protocol (MCP) Server** that exposes enterprise risk-management platform services to conversational AI agents. Built with the official Python `FastMCP` framework, the server acts as an intelligent middleware bridge, translating natural language requests into safe, stateful validation and remediation workflows.

This repository implements **Horizon 1 (The Minimum Viable Orchestrator)** and features a fully persistent **Stateful In-Memory Sandbox** engine, enabling immediate demonstration of closed-loop vulnerability identification, exploit verification, and remediation validation.
...
The diagram below details the end-to-end sequence showing how the AI Assistant collaborates with the MCP Server to identify, validate, approve, and remediate a live vulnerability within standard communication pipelines:
...
sequenceDiagram
    ...
    box rgb(30, 41, 59) MCP Server Layer
        participant Server as server.py (FastMCP)
        participant Registry as Stateful In-Memory DB
    end
    box rgb(15, 23, 42) Downstream Risk Platform Infrastructure
        participant TC as Validation Engine
        participant Eliminate as Remediation Engine (Platform Agent)
    end
    ...
    Registry-->>Server: Return Risk Profile (Risk score: 88, CVE Active)
    ...
    Server->>TC: Trigger validation check
    ...
    Server->>Eliminate: Deploy patch via platform agent (RAM: 4.8MB, CPU: <0.5%)
    Eliminate-->>Server: Remediation completed
    Server->>Registry: Mutate state (CVE status -> 'Remediated', risk score -> 30)
    Server-->>AI: Output success metrics & risk reduction delta (88 -> 30, -65% Risk)
    
    AI->>Engineer: "Vulnerability resolved on app-server-01. Exploit closed, risk score lowered to 30. Ticket #CC-8842 updated."
```
...
* **`telemetry://active-session-logs/{asset_id}`**: Retrieves real-time security posture, current risk score (0-100), host metadata (IP, OS, Environment), and active vulnerabilities. If the asset does not exist, the server dynamically generates a realistic telemetry profile.
...
* **`delegate_validation_workflow(asset_id: str, cve_id: str)`**: Invokes the platform's **exploit validation engine** to perform a safe, non-destructive cryptographic check. Returns an exploit verification response containing HTTP payloads or execution proof, mutating the in-memory vulnerability state to `Validated`.
* **`orchestrate_remediation(asset_id: str, action_type: str, change_control_id: str)`**: Performs stateful remediation via the platform's **remediation agent** (supporting `patch`, `mitigate_config`, or `network_isolate`).
...
- **Telemetry cards** for `app-server-01`, `db-host-05`, and `dev-box-09`, seeded with the same risk scores and CVEs as `server.py`'s `MOCK_ASSET_REGISTRY`
- **Validate Exploit** — runs the same probability the real tool does (85% verified for Production hosts, 50% for Development), and shows the matching exploit-validation evidence template either way
...
- **Authorize Patch Execution** — recalculates the risk score with the real formula (`max(15, score × 0.35)`) and reflects it live in both the dashboard and chat sidebar
...
To allow your Claude Desktop client to use this MCP Server, add the following snippet to your Claude configuration file:
